...
| `api/RecognizerRegistry` | Engines by id. Builtin: `vosk-text`, `ipa-phonemes`, `noop`. |
...
| `client/hud/VoiceCastHud`, `HudAnchor` | HUD rendering (waveform, transcript, localized status line) and screen anchors. |
| `engine/VoskTextRecognizer` | Vosk word engine (grammar mode with full-graph fallback), id `vosk-text` (default). |
